...
def applycal_wrapper(vis, target, band, solint, selfcal_library, current=lambda f: False, final=lambda f: False, 
    ...
    for fid in [fid for fid in np.intersect1d(selfcal_library[target][band]['sub-fields'],
            list(selfcal_library[target][band]['sub-fields-fid_map'][vis].keys())) if current(fid)]:
        applycal(vis=vis,\
    ...
    for fid in [fid for fid in np.intersect1d(selfcal_library[target][band]['sub-fields'],
            list(selfcal_library[target][band]['sub-fields-fid_map'][vis].keys())) if final(fid)]:
    ...
    for fid in [fid for fid in np.intersect1d(selfcal_library[target][band]['sub-fields'],
            list(selfcal_library[target][band]['sub-fields-fid_map'][vis].keys())) if clear(fid)]:

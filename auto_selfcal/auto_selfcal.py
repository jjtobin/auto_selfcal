    ...
    **kwargs : dict, optional
    ...
    for target in selfcal_library:
     ...
     for band in selfcal_library[target]:
       ...
       # we can get an estimate of the NF modifier for the auto-masking thresholds. To do this, we need to create a very basic mask
       # with the dirty image. So we just use one iteration with a tiny gain so that nothing is really subtracted off.
       tclean_return, theoretical_sensistivity = tclean_wrapper(selfcal_library[target][band],sani_target+'_'+band+'_dirty',
                      band,nsigma=4.0, scales=[0],
                      threshold='0.0Jy',niter=1, gain=0.00001,
                      savemodel='none',parallel=parallel,
                      field=target)
       ...
       tclean_return, theoretical_sensistivity = tclean_wrapper(selfcal_library[target][band],sani_target+'_'+band+'_initial',
                      band,nsigma=4.0, scales=[0],
                      threshold='theoretical_with_drmod',
                      savemodel='none',parallel=parallel,
       ...
       selfcal_library[target][band]["theoretical_sensitivity"] = theoretical_sensistivity
       selfcal_library[target][band]["theoretical_sensitivity_orig"] = theoretical_sensistivity
       for fid in selfcal_library[target][band]['sub-fields']:
           ...
           selfcal_library[target][band][fid]["theoretical_sensitivity"] = theoretical_sensistivity
           selfcal_library[target][band][fid]["theoretical_sensitivity_orig"] = theoretical_sensistivity
    ...
    for target in selfcal_library:
     ...
     for band in selfcal_library[target].keys():
       ...
       tclean_return, theoretical_sensistivity = tclean_wrapper(selfcal_library[target][band],sani_target+'_'+band+'_final',\

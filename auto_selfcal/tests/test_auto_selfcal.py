...
def test_benchmark(tmp_path, dataset):
    ...
    difference_count = compare_two_dictionaries(selfcal_library1, selfcal_library2, tolerance=1e-3, exclude=['vislist_orig','field_str','imsize'])
...
@pytest.mark.parametrize(
    "zip_file,link",
    [
        pytest.param("2018.1.01284.S_HOPS-384.tar.gz", 'https://nrao-my.sharepoint.com/:u:/g/personal/psheehan_nrao_edu/Ea8NGWjlNptNnyqg_62xxmcB0lk64IpB7Dd7AgfltnNkXQ?e=dHQ3sh&download=1', id="2018.1.01284.S_HOPS-384"),
...
def test_on_github(tmp_path, request, zip_file, link):
    ...
    difference_count = compare_two_dictionaries(selfcal_library1, selfcal_library2, tolerance=0.001, exclude=['vislist_orig','field_str','imsize'])

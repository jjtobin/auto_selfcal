      ...
      for solint in solints[band]:
         ...
         if solint == 'inf_EB':
            ...
            selfcal_library[target][band]['per_EB_SNR']=np.mean(SNR_self_EB)
               ...
               selfcal_library[target][band]['per_scan_SNR']=selfcal_library[target][band]['SNR_orig']/((n_ant-3)**0.5*(selfcal_library[target][band]['Total_TOS']/(selfcal_library[target][band]['Median_scan_time']/selfcal_library[target][band]['Median_fields_per_scan']))**0.5)
               solint_snr[target][band][solint]=selfcal_library[target][band]['per_scan_SNR']
               for spw in selfcal_library[target][band][vislist[0]]['spwsarray']:
...
def get_SNR_self_update(all_targets,band,vislist,selfcal_library,n_ant,solint_curr,solint_next,integration_time,solint_snr):
   for target in all_targets:
      if solint_next == 'inf' or solint_next == 'inf_ap':
         selfcal_library[target][band]['per_scan_SNR']=selfcal_library[target][band][vislist[0]][solint_curr]['SNR_post']/((n_ant-3)**0.5*(selfcal_library[target][band]['Total_TOS']/(selfcal_library[target][band]['Median_scan_time']/selfcal_library[target][band]['Median_fields_per_scan']))**0.5)
